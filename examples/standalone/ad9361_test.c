...
{
	printf("Usage: %s <rfic (0-3)>\n", command);
}

int ad9361_test (int argc, char * const argv[])
{
	int32_t rcode = 0;
	uint32_t bus = 0;
	char *cp = 0;
	...
	if(0 == rcode){
		/*
		 * TODO: Add ADC/DAC lines setup
		 *
		 */

		/*
		 * TODO: Add loopback test code
		 * ASIC design document is missing
		 * definitions for registers involved
		 * into data transfer setup
		 */
	...
	if(phy_ptr){
		...
		if(phy_ptr->adc_state)
			free(phy_ptr->adc_state);
		if(phy_ptr->adc_conv)
			free(phy_ptr->adc_conv);
		if(phy_ptr->pdata)
			free(phy_ptr->pdata);
		if(phy_ptr->clk_refin)
			free(phy_ptr->clk_refin);
		if(phy_ptr->spi)
			free(phy_ptr->spi);
